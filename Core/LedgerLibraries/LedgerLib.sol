    ...
    )
{
    ...
    uint256 isc = MarketManagementLib.isDMM(mmId, marketId)
        ? s.syntheticCollateral[marketId]
        : 0;

    freeCollateral = s.freeCollateral[mmId] + isc;

    int256 netAlloc = SolvencyLib._netUSDCAllocationSigned(s, mmId, marketId);

    virtualOffset = netAlloc + s.layOffset[mmId][marketId];
...
function getAvailableShares(uint256 mmId, uint256 marketId, uint256 positionId)
    ...
    returns (int256)
{
    (uint256 freeCollateral, int256 virtualOffset, int128 tilt) =
        getPositionLiquidity(mmId, marketId, positionId);
